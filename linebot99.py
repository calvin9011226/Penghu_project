...
log = Log(Auto_Clear=True,File_Only=False,choose=["no"])
timer = CodeTimer()

# 新增 safe_reply() 函式，避免使用無效的 reply token 拋出錯誤
def safe_reply(reply_token, messages,source="未指定功能"):
    try:
        dtype, size = Analyze.analyze_input(messages)
        log.data_size(size, message=f" reply_message <{source}> \t回傳大小", data_type=dtype)
        #log.data_size(len(str(messages).encode('utf-8')), message=f" reply_message <{source}> \t回傳大小", data_type="json")
        #log.data_message(f"回傳內容：{str(messages)}\n") 
...
def safe_push(user_id, messages,source="未指定功能"):
    try:
        dtype, size = Analyze.analyze_input(messages)
        log.data_size(dtype, message=f" push_message  <{source}> \t回傳大小", data_type=size)
        #log.data_size(len(str(messages).encode('utf-8')), message=f" push_message  <{source}> \t回傳大小", data_type="json")
        #log.data_message(f" 回傳內容：{str(messages)}\n")
...
def process_travel_planning(option, reply_token, user_id):
    ...
    try:
        ...
        safe_push(user_id, TextSendMessage("行程資料讀取失敗，請稍後再試"),"plan檔案讀取失敗")
        return
    ...
    try:
        ...
        safe_push(user_id, TextSendMessage("行程推薦失敗，請稍後再試"),"XGboost_plan 執行失敗")
        return
    ...
    try:
        ...
        print(f"資料處理失敗: {e}")
        safe_push(user_id, TextSendMessage("行程資料處理失敗，請稍後再試"),"行程資料處理失敗")
        return
    ...
    try:
        ...
        safe_push(user_id, TextSendMessage("無法取得您的位置，請重新傳送位置資訊"),"取得定位失敗")
        return
    ...
    safe_push(user_id, response_list,"路線選項說明")
...
def linebot_route():
    ...
    try:
        ...
        #節點
        dtype, size = Analyze.analyze_input(json_data['events'])
        #print(f"資料型態：{dtype}，大小：{size} bytes")
        #log.data_message("<event>")
        
        #log.data_size(size,data_type=dtype,message="event")   
        #log.data_time(timer.stop("event"),message="event") 
                ...
                if msg_type == 'text':
                    ...
                    if msg in ["系統路線", "使用者路線"]:
                        ...
                        try:
                            ...
                            print(f"無法取得使用者位置: {e}")
                            size_content=safe_reply(tk, TextSendMessage("無法取得您的位置，請重新傳送位置資訊"),"定位取得失敗")
                            return 'OK'
                        ...
                        size_content=safe_reply(tk, TextSendMessage(text=url),"系統與使用者路線功能")
                        timer_stop_log(tag=msg, size=size_content, timer=timer, log=log)
                        ...
                        try:
                            ...
                            if 0 <= int(msg) <= 120:
                                ...
                                message = FlexMessage.gender_reply("性別類型", "請輸入您的性別", 
                                                                   ...
                                                                   "女", "女", "女", 
                                                                   "其他", "其他", "其他")
                                size_content=safe_reply(tk, message,"年齡設定")
                                timer_stop_log(tag=msg, size=size_content, timer=timer, log=log)
                                approveGender = True
                            else:
                                print(f"data overflow or underflow: {msg}")
                                safe_reply(tk, TextSendMessage("請輸入\"正確年紀\""),"年齡錯誤")
                                size_content=timer_stop_log(msg, "年齡錯誤")
                    ...
                    if msg in ["兩天一夜", "三天兩夜", "四天三夜", "五天四夜"]:
                        response_list=process_travel_planning(msg, tk, user_id)
                    ...
                    if msg == "行程規劃" or msg == "6":
                        ...
                        response_list = [
                            TextSendMessage(f"感謝等待\n系統以AI大數據機器學習的方式推薦以下適合您的地點\n\n{recommend+clowd_message}"),
                        ...
                        timer_stop_log(tag="一般景點推薦", content=response_list, timer=timer, log=log)
                        # ************************************************************************************************
                        ...
                        # 取得不推薦的景點清單與訊息
                        dont_go_here, message_text = people_high5(tk)  # 人潮太多, 不推薦
                        dtype, size = Analyze.analyze_input(message_text)
                        log.data_size(size,data_type=dtype,message="event")   
                        print(message_text)
                        ...
                        response_list = [TextSendMessage(f"感謝等待\n系統以AI大數據機器學習的方式推薦以下適合您的地點\n\n"+str(recommend+clowd_message)),
                            ImageSendMessage(original_content_url=str(recommend_imgur)+".jpg",preview_image_url=str(recommend_imgur)+".jpg"),
                            TextSendMessage(recommend_website),
                            TextSendMessage(recommend_map),
                            TextSendMessage(str(message_text))
                        ...
                        line_bot_api.reply_message(tk,response_list)
                        timer_stop_log(tag="永續觀光", content=response_list, timer=timer, log=log)
                        ...
                        try:
                            ...
                            print(f"❌ 附近搜尋功能錯誤: {e}")
                            #size_content=safe_reply(tk, TextSendMessage("附近搜尋功能錯誤，請稍後再試"),"附近搜尋功能錯誤")
                            safe_reply(tk, TextSendMessage("附近搜尋功能錯誤，請稍後再試"),"附近搜尋功能錯誤")
                            #timer_stop_log(tag=msg, size=size_content, timer=timer, log=log)
                            return 'OK'
                    elif msg == "景點":
                        print("景點推薦網址功能觸發")
                        ...
                        print("收集資料功能觸發")
                        size_content=safe_reply(tk, TextSendMessage("請輸入你的年紀"),msg)
                        timer_stop_log(tag=msg, size=size_content, timer=timer, log=log)
                    ...
                    try:
                        ...
                        size_content=safe_reply(tk, TextSendMessage("資料儲存完畢\n請根據您的需求點選下方圖文選單,來獲得相對應的功能"),"寫入location")
                        timer_stop_log(tag=msg_type, size=size_content, timer=timer, log=log)
                        print("結束使用「收集資料功能」\n------------------")
                    except Exception as e:
                        print(f"❌ 位置資料儲存錯誤: {e}")
                        size_content=safe_reply(tk, TextSendMessage("位置資料儲存錯誤，請稍後再試"),"location寫入失敗")
                        timer_stop_log(tag=msg, size=size_content, timer=timer, log=log)
            else:
...
def handle_postback(event):
    ...
    if postback_data in ["兩天一夜", "三天兩夜", "四天三夜", "五天四夜"]:
        ...
        try:
            ...
            print(f"無法取得使用者位置: {e}")
            safe_reply(event.reply_token, TextSendMessage("無法取得您的位置，請重新傳送位置資訊"),"使用者位置取得失敗")
            return
...
def generate_timeline():
    timer.generate_timeline_plot()
    timer.Function_duration(detail_timestamp=True)
    return "✅ 時序圖已建立"
